...
const menuGauge = new promClient.Gauge({ name: "menu_status", help: "check status of menu, 0 is success, 1 is failure", labelNames: ["prod", "phase"] });
...
export const pushMenuStatus = (prodInfo: string, phase: string, val: number) => {
  if (isMaintaining()) return
